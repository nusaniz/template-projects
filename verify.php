...
$upload_dir = 'uploads/files_verification/';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        $upload_file = $upload_dir . $file_base_name; // Full path for the file
        ...
        if (move_uploaded_file($file_tmp_name, $upload_file)) {
            ...
            // Check if the file hash exists in the tb_file table and its status is 'active'
            $stmt = $conn->prepare("
                SELECT 
                ...
                WHERE f.file_hash = ? AND f.status = 'active'
            ");
            ...
            // Insert the file hash and new file name into tb_file_verification
            $stmt = $conn->prepare("
                INSERT INTO tb_file_verification (file, file_hash, status, created_at)
                VALUES (?, ?, ?, NOW())
            ");
            $stmt->bind_param("sss", $file_base_name, $file_hash, $status);

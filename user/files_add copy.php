...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($file_tmp_name, $upload_file)) {
            ...
            $stmt = $conn->prepare("
                INSERT INTO tb_file (file_name, file, file_hash, divisi_id, jabatan_id, status, user_id, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
            ");
            $stmt->bind_param("ssssssi", $file_name, $file_base_name, $file_hash, $divisi_id, $jabatan_id, $status, $user_id);

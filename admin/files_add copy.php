...
if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
    ...
    // $fileBaseName = basename($fileOriginalName) . '_' . $uniquePrefix; // Rename file with prefix
    ...
    if (move_uploaded_file($fileTmpPath, $destPath)) {
        ...
        $stmt = $conn->prepare("
            INSERT INTO tb_file (user_id, file_name, file, file_hash, divisi_id, jabatan_id, status) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->bind_param("issssss", $user_id, $file_name, $fileBaseName, $fileHash, $divisi_id, $jabatan_id, $status);
